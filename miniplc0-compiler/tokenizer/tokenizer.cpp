	...
	std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
                ...
                char ch = current_char.value(); //当前读到的字符
				...
				if (!current_char.has_value()){
                    ...
                    //如果解析结果是关键字，那么返回对应关键字的token，否则返回标识符的token
                   	...
                   	if (val == "begin") {
						...
						return std::make_pair(std::make_optional<Token>(TokenType::BEGIN, val, pos, currentPos()), std::optional<CompilationError>());

					}

					else if (val == "end")

						return std::make_pair(std::make_optional<Token>(TokenType::END, val, pos, currentPos()), std::optional<CompilationError>());

					else if (val == "var")

						return std::make_pair(std::make_optional<Token>(TokenType::VAR, val, pos, currentPos()), std::optional<CompilationError>());

					else if (val == "const")

						return std::make_pair(std::make_optional<Token>(TokenType::CONST, val, pos, currentPos()), std::optional<CompilationError>());

					else if (val == "print")

						return std::make_pair(std::make_optional<Token>(TokenType::PRINT, val, pos, currentPos()), std::optional<CompilationError>());

					else

						return std::make_pair(std::make_optional<Token>(TokenType::IDENTIFIER, val, pos, currentPos()), std::optional<CompilationError>());
