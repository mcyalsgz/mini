...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while (true) {// 预读
            auto next = nextToken();//预读下一个Token
            bool initialed_flag=false;
            if (!next.has_value())
                return {};
            ...
            next = nextToken();
            if (!next.has_value() || next.value().GetType() != TokenType::IDENTIFIER)
                return std::make_optional<CompilationError>(next.value().GetStartPos(), ErrorCode::ErrNeedIdentifier);
            if (isDeclared(next.value().GetValueString()))
                return std::make_optional<CompilationError>(next.value().GetStartPos(), ErrorCode::ErrDuplicateDeclaration);
            ...
            next = nextToken();
            if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
                return std::make_optional<CompilationError>(next.value().GetStartPos(), ErrorCode::ErrNoSemicolon);
        }
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
        ...
        //'+' ,开头是加号的情况
        if (next.value().GetType() == PLUS_SIGN) {
                            next = nextToken();
                            flag = true;
                }

        //'-'，开头是减号的情况
        else if (next.value().GetType() == MINUS_SIGN) {
                    next = nextToken();
        ...
        //把字符串转化为整型数再变成负数
        out = atoi(std::any_cast<std::string>(next.value().GetValue()).c_str());
        if(!flag){
         out = out*(-1)；
         }
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
    	...
    	//生成相应的指令
            _instructions.emplace_back(Operation::STO, getIndex(id.GetValueString()));
            if(!isInitializedVariable(id.GetValueString()))
                addVariable(id);
		return {};
	}
